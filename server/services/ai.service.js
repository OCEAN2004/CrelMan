...
export const generateLeadSummary = async (lead) => {
  const prompt = `
You are an expert B2B sales analyst for a CRM called CRELMAN.

This CRM is used by Indian businesses.
Assume every monetary value is in Indian Rupees (₹ / INR), not US Dollars.
...
The CRM is CRELMAN and all business transactions use Indian Rupees (₹ / INR).
If you mention pricing or deal values, always use ₹ instead of $.
...
export const generateSalesInsights = async (pipelineStats) => {
  const prompt = `
You are a revenue operations advisor for CRELMAN.

This CRM is used by Indian businesses.
Assume every monetary value in the pipeline is in Indian Rupees (₹ / INR), not US Dollars.
